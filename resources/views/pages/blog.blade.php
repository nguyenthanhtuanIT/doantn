				...
				<div class="container">
					<div class="row">
						...
						<div class="col-md-12 breadcrumbs text-center">
							<h2>Tour of {{$c->name}} </h2>
							<p class="text_info">{{$c->email}} | Hotline: {{$c->phone}}</p>
							@endif
							@endforeach
							@if (Auth::check())
							<p class="text_info">Welcom {{Auth::user()->username}}</p>
							@endif
						</div>
					</div>
				</aside>
											...
											<form action="detail/{{$d->tour_id}}" method="post">
												...
												<div class="row form-group">
													...
													<div class="form-group">
														<button type="submit " style="margin-top:20px;"class="btn btn-primary" >Post comment</button>
													</div>
													@else
													<div class="alert alert-danger">Login to comment</div>
													@endif
												</form>
											</div>
										</div>
